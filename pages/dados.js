...
(function () {
    ...
    function renderCommunities(container, items) {
        ...
        items.forEach(function(item){var a=document.createElement("a");a.className="card dynamic-item";a.href="../comunidade-especifica/index.html?id="+encodeURIComponent(idOf(item));a.innerHTML='<div class="spread"><strong>'+esc(item.nome||"")+'</strong><span class="status">'+esc(item.status||"")+'</span></div><p class="meta">'+esc(item.descricao||"")+'</p>';container.appendChild(a);});
    }
